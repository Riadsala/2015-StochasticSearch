...
%     meancoverage(beta) = mean(trialcoverage);
%     meansaccades(beta) = mean(results(isfinite(results)));
%     stdsaccades(beta) = std(results(isfinite(results)));
     meanrefix(beta) = mean(numrefix./results);
end
...
% errorbar(repmat([1.6,1.65,1.7], [3,1])', meansaccades', stdsaccades'./sqrt(numbertrials));
