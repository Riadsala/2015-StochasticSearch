...
for beta = beta_Set
    for ctr = 1:NumberOfExamplesPerComb
        ...
        for RMS = RMS_Set
            for r = r_means
                overallctr = overallctr+1;
                T = csvread([int2str(seed) '.csv']);
                T = T.*RMS;
                filename = ['RMS=' num2str(RMS) '_beta=' num2str(beta) '_r=' int2str(r) '_seed=' int2str(seed)];
                % generate random coordinates for defect placement
                [x y] = RandomCoordinates(r, r_range, N);
               
                coords = [coords; RMS, beta, seed, x, y];
                defect_positions(x,y) = 1;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % create defect
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
toc
csvwrite('locations.csv', coords);
imwrite(defect_positions, 'defectpositions.jpg');
end
...
function [x y] = RandomCoordinates(r,r_range, N)
r = (r_range(2)-r_range(1)).*rand+r_range(1)+r;
phi = rand*2*pi;
[x,y] = pol2cart(phi, r);
x = round(x+N/2);
y = round(y+N/2);
